...
#>
	...
	[regex]$ipv4="\b^[^:](?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\b"
	...
	[string[]]$ips=@()
	if (-not($ipv4only)) {
		$ips+=((get-content $file | Select-String -Pattern "$ipv6" -AllMatches).Matches.Value) -like "*:*"
	}
	if (-not($ipv6only)) {
		$ips+=((get-content $file | Select-String -Pattern "$ipv4" -AllMatches).Matches.Value)
	}

	$ips=$ips.replace(" ","")
	$ips=$ips|select-object -unique|sort

	if ($ips -notlike "*false*") {
		$ips
	} else {
		"No IP addresses in the file."
	}
}
